layui.define( ['table', 'admin', 'form', 'view', 'laydate', 'element'], function(exports){
    ...
    let initFormName = 'email_search_traceless_player_send_form';   // 初始化搜索框的name
    let initFormSel = '#email_mass_send_form';  // 搜索框所在div的id
    let formDivClassSel = '.email_mass_send_form';
    let paramType = 4;
    let searchBtnSel = '#search_mass_send';
    let resetBtnSel = '#reset_mass_send';
    let tableId = 'mass_list';
    ...
    initForm(initFormName, initFormSel).then(function () {
        ...
        table.render({
            id: tableId,
            elem: '#' + tableId,
            url: admin.getUrl('/api/traceless/mass/getEmailList'),
            method: 'GET',
            where: param,//请求参数(额外)
            request: {
    ...
    exports('traceless/email_mass_traceless', {})
