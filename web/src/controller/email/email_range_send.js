layui.define( ['table', 'admin', 'form', 'view', 'element', 'common'], function(exports){
    ...
    let initFormName = 'email_search_range_send_form';   // 初始化搜索框的name
    let initFormSel = '#email_range_send_form';  // 搜索框所在div的id
    let formDivClassSel = '.email_range_send_form';
    let paramType = 3;
    let searchBtnSel = '#search_range_send';
    let resetBtnSel = '#reset_range_send';
    let tableId = 'range_list';
    ...
    initForm(initFormName, initFormSel).then(function () {
        ...
        table.render({
            id: tableId,
            elem: '#' + tableId,
            url: admin.getUrl('/api/email/range/getEmailList'),
            method: 'GET',
            where: param,//请求参数(额外)
            request: {
    ...
    exports('email/email_range_send', {})
